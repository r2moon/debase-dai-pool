...
contract('DaiLpPoolV2 Mainnet testing', (accounts) => {
  ...
  it('Check deposit', async () => {
    ...
    await expectRevert(
      daiLpPool.withdrawMphVested(1, {from: accounts[1]}),
      "no deposit"
    );
    await expectRevert(
      daiLpPool.withdrawMphVested(0, {from: accounts[8]}),
      "not owner"
    );
    await expectRevert(
      daiLpPool.withdrawMphVested(0, {from: accounts[1]}),
      "Not ready to withdarw mph"
    );
    await time.increase(config.vestingPeriod);
    const mphUserOldBalance = new BN((await mph.balanceOf(accounts[1])).toString());
    const mphTreasureOldBalance = new BN((await mph.balanceOf(treasury)).toString());

    await daiLpPool.withdrawMphVested(0, {from: accounts[1]});
    const mphUserBalance = new BN((await mph.balanceOf(accounts[1])).toString());
    const mphPoolBalance = new BN((await mph.balanceOf(daiLpPool.address)).toString());
    const mphTreasureBalance = new BN((await mph.balanceOf(treasury)).toString());
    const deposit = await daiLpPool.deposits(0);
    const mphReward = new BN(deposit['mphReward'].toString())
    assert.equal(
      mphUserBalance.sub(mphUserOldBalance).add(mphPoolBalance)
      .add(mphTreasureBalance).sub(mphTreasureOldBalance).toString(),
      mphReward.toString()
    );
    assert.equal(
      mphPoolBalance.toString(),
      mphReward.mul(new BN(config.mphTakingBackMultiplier)).div((new BN("10").pow(new BN("18")))).toString()
    );
    assert.equal(
      mphTreasureBalance.sub(mphTreasureOldBalance).toString(),
      mphReward.sub(mphPoolBalance).mul(new BN(config.mphFee.toString())).div((new BN("1000"))).toString()
    );
